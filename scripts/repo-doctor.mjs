...
const IGNORE_DIRS = new Set(["node_modules", ".next", ".git", ".turbo", "dist", "build", ".vercel"]);
...
async function readJSON(p) {
  ...
  catch (e) { LOG.err(`Invalid JSON at ${p}: ${e.message}`); return null; }
}
...
async function main() {
  LOG.info("Scanning repo from", rel(process.cwd()), "with deployment root", `"${rel(ROOT)}"`);
  ...
  const nextConfigs = files.filter(f => /(^|\/)next\.config\.(js|cjs|mjs|ts)$/i.test(f));
  ...
  if (!hasRootVercel) {
    ...
    LOG.warn("Vercel will ignore vercel.json outside the Root Directory. Move/copy the modern config here.");
  }
  ...
  if (!hasRootPkg) LOG.err(`No package.json at deployment root: ${rel(rootPkg)}.`);
  else {
    const pkg = await readJSON(rootPkg);
    if (pkg?.packageManager?.startsWith("pnpm@")) LOG.ok(`packageManager is set to ${pkg.packageManager}.`);
    ...
    else LOG.warn("No Next.js dependency found in package.json at deployment root.");

    if (pkg?.scripts?.build !== "next build") {
      LOG.warn(`build script is "${pkg?.scripts?.build}" — recommend "next build".`);
    } else {
  ...
  if (nextConfigs.length === 0) {
    ...
    if (!foundIgnore) LOG.warn("ESLint ignoreDuringBuilds not detected; CI may fail on missing ESLint presets.");
  }
  ...
  if (await exists(appPage)) {
    ...
    const hasDynamic = /export\s+const\s+dynamic\s*=\s*["'`]force-dynamic["'`]/.test(txt);
    if (hasRedirect) LOG.ok("(app)/page.tsx redirects to /dashboard.");
    if (!hasDynamic) LOG.warn("(app)/page.tsx missing export const dynamic = \"force-dynamic\";");
  } else {
    LOG.warn(`Missing ${rel(appPage)} — not fatal, but recommended for deterministic builds.`);
  }
  ...
  if (APPLY_FIX) {
    ...
    if (hasRootVercel) {
      ...
      LOG.warn("Cannot write modern vercel.json: no vercel.json at deployment root. Create it manually or set Vercel Root Directory to the correct folder.");
    }
    ...
    for (const lf of legacyBuildsPaths) {
      ...
      if (p !== candidateVercel) {
        LOG.warn(`Legacy vercel.json not at deployment root: ${rel(p)} — consider deleting or updating to avoid confusion.`);
      }
    }
    ...
    if (!hasRootLock && lockFiles.length) {
      ...
      for (const lf of lockFiles) {
        ...
        if (score < bestScore) { best = lf; bestScore = score; }
      }
      if (best) {
  ...
  console.log("\n--- SUMMARY ---");
  if (!hasRootVercel) LOG.err(`• Missing vercel.json at deployment root (${rel(candidateVercel)}).`);
  if (legacyBuildsPaths.length) LOG.warn("• Legacy vercel.json with \"builds\" detected (remove or replace).");
  if (!hasRootLock) LOG.warn(`• Missing pnpm-lock.yaml at deployment root (${rel(rootLock)}).`);
  LOG.info("• Ensure Vercel Project → Settings → General → Root Directory points to:", rel(ROOT));
