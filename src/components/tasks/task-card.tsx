...
import { CalendarDays, ChevronRight, Clock, Trash2, CheckCircle2, AlarmClock } from "lucide-react";
...
export function TaskCard({
  ...
  const p = task.priorityScore ? (Number(task.priorityScore) as 1|2|3|4) : 2;
    ...
    >
      ...
      <div className="px-4 py-3 pl-5">
        ...
        <div className="mt-2 flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
          {due && (
            <span className={cn("inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full",
              overdue ? "bg-red-500/10 text-red-600" : "bg-black/5 dark:bg-white/10"
            )}>
          ...
          {task.role && (
            <span
              ...
              style={{ backgroundColor: `${task.role.color}22`, color: task.role.color }}
              title="Role"
            >
              {task.role.name}
            </span>
      ...
      <div className="absolute right-2 top-2 flex gap-1 opacity-0 group-hover:opacity-100 transition">
        <IconBtn label="Complete" onClick={() => onComplete?.(task.id)}><CheckCircle2 className="h-4 w-4" /></IconBtn>
        <IconBtn label="Snooze +1d" onClick={() => onSnooze?.(task.id,1)}><AlarmClock className="h-4 w-4" /></IconBtn>
        <IconBtn label="Delete" onClick={() => onDelete?.(task.id)}><Trash2 className="h-4 w-4" /></IconBtn>
      </div>
    </motion.div>
  );
}
...
function dueBadge(due: Date|null){
  ...
  const dueText = diff === 0 ? "Today" : diff === 1 ? "Tomorrow" : diff > 1 ? `In ${diff}d` : `${Math.abs(diff)}d late`;
  const chip = (
    <span className={cn(
