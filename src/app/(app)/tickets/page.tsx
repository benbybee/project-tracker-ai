...
export default function TicketsPage() {
  ...
  const uniqueProjects = Array.from(
    new Set(tickets.map((t) => t.projectName))
  ).filter(Boolean);
  ...
  const filteredTickets = tickets
    .filter((ticket) => {
      if (statusFilter !== 'all' && ticket.status !== statusFilter)
        return false;
      if (projectFilter !== 'all' && ticket.projectName !== projectFilter)
        return false;
      if (dateFilter !== 'all') {
        const ticketDate = new Date(ticket.createdAt);
        const now = new Date();
        const daysDiff = Math.floor(
          (now.getTime() - ticketDate.getTime()) / (1000 * 60 * 60 * 24)
        );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="rounded-xl border bg-white/80 backdrop-blur p-4 mb-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex lg:flex-wrap gap-3 lg:gap-4">
            ...
            <div className="flex items-center gap-2 min-w-0">
              <label className="text-sm font-medium flex-shrink-0">Date:</label>
              <select
                value={dateFilter}
            ...
            <div className="hidden sm:flex items-center gap-2">
              <label className="text-sm font-medium flex-shrink-0">Sort:</label>
              <select
                value={sortBy}
                ...
                {filteredTickets.map((ticket) => (
                  <TicketRow
                    ...
                    onDelete={async () => {
                      if (
                        confirm(
                          'Are you sure you want to delete this ticket? This will also delete all associated tasks.'
                        )
                      ) {
                        try {
...
function TicketRow({
  ...
  return (
    <tr className="hover:bg-gray-50">
      <td className="px-4 py-3 text-sm text-gray-900">{ticket.projectName}</td>
      ...
      <td className="px-4 py-3 text-sm text-gray-600 relative">
        <div className="relative">
          ...
          >
            {assignedProject
              ? availableProjects.find((p) => p.id === assignedProject)?.name ||
                'Unknown Project'
              : 'Assign'}
          </button>
          {showProjectDropdown && (
      ...
      <td className="px-4 py-3 text-sm text-gray-600">{completionDate}</td>
      <td className="px-4 py-3">
        <span
          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
            ticket.status === 'new'
              ...
              <div>
                <div className="text-sm text-gray-700">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {ticket.domain || '—'} · Priority:{' '}
                    <span className="font-medium">{ticket.priority}</span>
                    {ticket.dueDateSuggested &&
                      ` · Requested: ${ticket.dueDateSuggested}`}
                  </div>
                </div>
              </div>
              <div className="flex gap-2">
              ...
              <section className="rounded-lg border p-3">
                ...
                <div className="space-y-3">
                  {proposed.map((p, idx) => (
                    <div
                      key={p.id}
                      className="grid grid-cols-1 lg:grid-cols-[1fr_200px_100px_80px] gap-3 rounded-lg border p-3 hover:bg-gray-50"
                    >
                      <div>
    ...
    <div className="rounded-xl border bg-white/80 backdrop-blur p-4 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1 min-w-0">
          ...
          {ticket.domain && (
            <p className="text-sm text-gray-600 mb-2">🌐 {ticket.domain}</p>
          )}
          <div className="text-xs text-gray-500 space-y-1">
            <p>Due: {completionDate}</p>
          </div>
        </div>
      </div>
