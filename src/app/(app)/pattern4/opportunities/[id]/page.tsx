...
export default function OpportunityDetailPage({
  ...
  const chartData = tasks.reduce((acc: any[], task) => {
    // Simple mock trend for now - in production this would aggregate task history
    return acc; 
  }, []);
  ...
  const createTask = trpc.pattern4.tasks.create.useMutation({
    onSuccess: () => {
      utils.pattern4.tasks.listByOpportunity.invalidate({ opportunityId: resolvedParams.id });
      utils.pattern4.stats.opportunityProgress.invalidate({ opportunityId: resolvedParams.id });
    },
  });
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <AIActionButton 
            label="AI: Analyze" 
            prompt={`Analyze opportunity "${opportunity.name}". Evaluate viability, risks, and next steps.`}
            ...
            <TaskList
              ...
              onTaskUpdate={async (taskId, data) => {
                await updateTask.mutateAsync({
                  ...
                  priority: data.priorityScore ? parseInt(data.priorityScore) : undefined,
