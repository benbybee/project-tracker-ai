...
export async function getDB() {
  if (!isBrowser()) {
    throw new Error(
      'getDB() called on server - this should only run in the browser'
    );
  }
  ...
  if (!dbPromise) {
    dbPromise = (async () => {
      ...
      class TaskTrackerDB extends Dexie {
        ...
        constructor() {
          ...
          this.version(4).stores({
            ...
            offlineMessages:
              'id, threadId, content, messageType, metadata, retryCount, createdAt',
          });
        ...
        async getSyncStatus() {
          return (
            (await this.syncStatus.get('main')) || {
        ...
        async markAllNotificationsAsRead() {
          await this.notifications.where('read').equals(false).modify({
            read: true,
            updatedAt: new Date().toISOString(),
            syncStatus: 'pending',
          });
        ...
        async getOfflineMessages() {
          return await this.offlineMessages.orderBy('createdAt').toArray();
        }
      ...
      try {
        ...
        console.error(
          'Failed to open database, attempting to delete and recreate:',
          error
        );
