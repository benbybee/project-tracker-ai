...
type Entity = Record<string, any> & { id: string; version?: number; updatedAt?: string };
...
function isNoop(localPatch: Record<string, any>, remote: Entity) {
  ...
  return Object.keys(localPatch).every(k => JSON.stringify(remote[k]) === JSON.stringify(localPatch[k]));
}
...
function canAutoMerge(localPatch: Record<string, any>, localBase: Entity, remote: Entity) {
...
export async function handleStaleVersion({
  ...
  if (isNoop(localPatch, remote)) {
    if (entityType === 'task') await updateLocalVersion(id, remote.version ?? 0);
  ...
  if (canAutoMerge(localPatch, localBase, remote)) {
    const rebased = { ...remote, ...localPatch, version: (remote.version ?? 0) + 1, updatedAt: new Date().toISOString() };
    const db = await getDB();
  ...
  return { resolved: false, mode: 'conflict' as const, openModal: open, remote, localPatch, localBase };
}
